	...
	for {
		...
		case cHead := <-voteManager.chainHeadCh:
			...
			if rand.Intn(100) < voteManager.pool.probNoVote {
				log.Info("vote skipped", "probNoVote", voteManager.pool.probNoVote)
				continue
			}
			if !startVote {
			...
			if ok, sourceNumber, sourceHash := voteManager.UnderRules(curHead); ok || rand.Intn(100) < voteManager.pool.probBreakVoteRule {
				if !ok {
					log.Info("vote rules check skipped", "probBreakVoteRule", voteManager.pool.probBreakVoteRule)
				} else {
			...
			if rand.Intn(100) < voteManager.pool.probBreakVoteRule {
				for i := 0; i < 5; i++ {
					curHead = voteManager.chain.GetHeaderByHash(curHead.ParentHash)
					if len(voteManager.pool.FetchVoteByBlockHash(curHead.Hash())) != 10 {
						vote := &types.VoteData{
							TargetNumber: curHead.Number.Uint64(),
							TargetHash:   curHead.Hash(),
						}
						...
						sourceNumber := curHead.Number.Uint64() - 1
						sourceHash := curHead.ParentHash

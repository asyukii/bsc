	...
	if s.db.snap == nil || err != nil {
		...
		if s.db.EnableExpire() {
			if enErr, ok := err.(*trie.ExpiredNodeError); ok {
				log.Info("GetCommittedState expired in trie", "addr", s.address, "key", key, "err", err)
...
func (s *stateObject) updateTrie() (Trie, error) {
	...
	if s.db.EnableExpire() {
		...
		for key := range s.pendingAccessedState {
			...
			log.Info("updateTrie access state", "contract", s.address, "key", key, "epoch", s.db.Epoch())
		}
	}
	...
	go func() {
		...
		for key, value := range dirtyStorage {
			if len(value) == 0 {
				...
				log.Info("updateTrie DeleteStorage", "contract", s.address, "key", key, "epoch", s.db.Epoch(), "value", value, "tr.epoch", tr.Epoch(), "tr", fmt.Sprintf("%p", tr), "ins", fmt.Sprintf("%p", s))
				...
				log.Info("updateTrie UpdateStorage", "contract", s.address, "key", key, "epoch", s.db.Epoch(), "value", value, "tr.epoch", tr.Epoch(), "tr", fmt.Sprintf("%p", tr), "ins", fmt.Sprintf("%p", s))
	...
	go func() {
		...
		for key, value := range dirtyStorage {
			...
			log.Info("updateTrie UpdateSnapShot", "contract", s.address, "key", key, "epoch", s.db.Epoch(), "value", snapshotVal, "tr.epoch", tr.Epoch(), "tr", fmt.Sprintf("%p", tr), "ins", fmt.Sprintf("%p", s))
...
func (s *stateObject) getExpirySnapStorage(key common.Hash) ([]byte, error, error) {
	...
	log.Info("GetCommittedState expired in snapshot", "addr", s.address, "key", key, "val", val, "err", err)
